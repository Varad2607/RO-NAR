...
from stretch_perception import get_target_position, target_present, detect_clean_dirty_clothes
...
class StretchActions(Node):
    def __init__(self):
        ...
        self.user_error_service=self.create_service(ServiceString, 'user_error', self.user_error_callback)
        ...
        self.position_mode_client.wait_for_service()
        self.navigation_mode_client.wait_for_service()
    ...
    def base_to_camera_tf_cb(self):
        try:
            self.base_to_camera_tf = self.tf_buffer.lookup_transform('base_link', 'camera_link', Time())
        except TransformException as e:
            self.get_logger().error(f"Transform error: {e}")


    def look_for_object(self, target_name='cloth', confidence=0.01, head_tilt=-0.5, pan_steps=9, left_limit=-3.6, right_limit=1.45):
    ...
    def get_target_position(self, target_class='cloth', confidence=0.01):
    ...
    def look_for_dirty_clothes(self, target_name, confidence=0.5, head_tilt=-0.5, pan_steps=9, left_limit=-3.6, right_limit=1.45):
        while self.rgb is None:
            time.sleep(0.1)
        ...
        probs=detect_clean_dirty_clothes(rgb_image, confidence)
        if probs is None:
            return 'fail'
    ...
    def switch_to_navigation_mode(self):
        #send request to switch to navigation mode
    ...
    def switch_to_position_mode(self):
        # send request to switch to position mode
        request = Trigger.Request()
        self.robot_ready = False
    ...
    def user_error_callback(self, request, response):
        self.user_error = True
        if request.data == 'nav':
            future = self.nav_goal_handle.cancel_goal_async()
            future.add_done_callback(self.cancel_nav_goal_callback)

        response.result = 'detected error'
        return response
